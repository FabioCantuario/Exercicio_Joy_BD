...
DROP TABLE aluno;
DROP FUNCTION adicionar_autor_default;
...
-- Criação da função para a trigger
CREATE OR REPLACE FUNCTION adicionar_autor_default()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.autorLivro IS NULL THEN
        NEW.autorLivro := 'Autor Desconhecido';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Criação da trigger
CREATE TRIGGER trigger_adicionar_autor
BEFORE INSERT ON livro
FOR EACH ROW
EXECUTE FUNCTION adicionar_autor_default();
